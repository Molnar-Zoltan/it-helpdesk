...
- **`PATCH /tickets/:id/status` — an explicit transition map, not a free-form status write.** `TicketsService.ALLOWED_AGENT_STATUS_TRANSITIONS` enumerates exactly which statuses each current status may move to: `OPEN ↔ IN_PROGRESS ↔ RESOLVED`, plus any of those three → `CLOSED` (reusing the same `closeReason`/`closedAt`/`closedBy` columns a customer-initiated close writes, with a required `reason` validated the same way), plus `CLOSED → OPEN`/`CLOSED → IN_PROGRESS` — an agent-driven reopen, added after Step 9.2 shipped once it became clear reopen shouldn't be customer-exclusive (e.g. an agent who closed a ticket by mistake, or one where the customer isn't available to reopen it themselves). Self-transitions are deliberately absent from the map, so a repeat call with the same target status `400`s instead of silently rewriting the row — same no-op-guarding instinct as the assign endpoint, enforced by the map's shape instead of an extra check. The customer's own `PATCH /tickets/:id/reopen` is untouched and stays fully independent — this is a parallel path, not a replacement. Reopening through this endpoint reuses `reopenReason`/`reopenedAt`/`reopenedBy`, the same columns the customer-reopen endpoint writes, with `reason` required (validated by hand in the service against `TICKET_REOPEN_REASON_MIN/MAX_LENGTH`, since the DTO only sees the target status, not the ticket's current one, so it can't know a reason is even needed the way it can for a `CLOSED` target). Permission is the assigned agent or `ADMIN` for every transition including reopen — closing a ticket doesn't clear `agentId`; an unassigned ticket is `ADMIN`-only, since there's no "assigned agent" to authorize yet.
...
- **Manual ticket creation is intentionally *not* gated by `isDemoUserId`.** The four self-service mutations under `/users/me` are demo-protected because they'd break a shared account every visitor relies on; filing a ticket does neither, so all four demo accounts can create tickets like any other user. Only the account tabs check `isDemoUserId`.
